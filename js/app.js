class VideoForgeApp {
  ...
  async generateVideo(prompt, duration) {
    ...
    if (!this.ffmpeg) {
      this.updateTips([
        '\u{1F4E6} Загрузка ffmpeg (\u224830 МБ)...',
        '\u23F3 Только при первом запуске',
        '\u{1F4A1} Пожалуйста, подождите'
      ]);
      this.ffmpeg = await this.loadFFmpeg();
    }

    this.updateTips([
      '\u{1F3A8} Рендеринг кадров...',
      `\u{1F5BC} 0/${totalFrames}`,
      '\u23F3 Подготовка...'
    ]);
    ...
    for (let f = 0; f < totalFrames; f++) {
      ...
      await this.ffmpeg.writeFile(`f${f}.raw`, new Uint8Array(imageData.data.buffer));
    ...
    for (let f = 0; f < totalFrames; f++) {
      try { await this.ffmpeg.deleteFile(`f${f}.raw`); } catch (e) {}
    }
